...
//-----------------------------------------------------------------------------
int vtkCUDAVolumeMapper::IsRenderSupported(
...
void vtkCUDAVolumeMapper::ClearInput()
{
  ...
  std::map<int, vtkImageData*>::iterator it;
  for( it = this->inputImages.begin(); it!=this->inputImages.end(); it++)
...
void vtkCUDAVolumeMapper::ComputeMatrices()
{
  ...
  if(flag)
    ...
    this->NextVoxelsToViewTransform->SetMatrix( this->VoxelsToViewTransform->GetMatrix() );
